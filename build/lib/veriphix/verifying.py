...
class ComputationRun(Run):
    ...
    def delegate(self, backend: Backend, **kwargs) -> ComputationResult:
        ...
        # If classical output, return the output (majority vote)
        if self.client.type_output == "rvbqc":
        ...
        ###! If VBOE, compute mean value
        elif self.client.type_output == "vboe":
            results = {onode: self.client.results[onode] for onode in self.client.output_nodes}
            return VBOEComputationResult(outcomes=results)
        ### TODO: implement VBPEC
        elif self.client.type_output == "vbpec":
            raise Exception("VBPEC has not been supported yet.")
...
class ClassicalComputationResult(ComputationResult):
    ...
    def analyze(self, result_analysis: ResultAnalysis, client: Client) -> None:
        ...
        result_analysis.computation_count += client.output_predicate(output_string) ###! decision problem only
        return
...
class VBOEComputationResult(ComputationResult):
    def __init__(self, 
    ...
    def analyze(self, 
                ...
                client: Client) -> None:
        output_string = "".join(str(int(v)) for v in self.outcomes.values())
        if not (output_string in result_analysis.hist.keys()):
            result_analysis.hist[output_string] = 1
        else:
            result_analysis.hist[output_string] += 1
        return
    
    def compute_expval(self,
                       result_analysis: ResultAnalysis, 
                      ):
        N_total = sum(result_analysis.hist.values()) # .shots()
        if N_total < 1:
            raise Exception("0 shot in histogram.")
        sum_with_weight = 0
        for key, value in result_analysis.hist.items():
            if key.count("1") & 1 == 1:
                sum_with_weight -= value
            else:
                sum_with_weight += value
        expval = sum_with_weight / N_total
        result_analysis.expval = expval
...
class ResultAnalysis:
    ...
    hist: dict = field(default_factory=dict) ###! big endian
    expval: float = None
